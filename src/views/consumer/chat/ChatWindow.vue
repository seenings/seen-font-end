            ...
            >
              <seen-self-sender-comp
                ...
                "
                  ...
                  @load-image="loadImage(localChatMessage.remoteId)"
              >
              </seen-self-sender-comp>
            </template>
            <template v-else>
...
import {API_VOICE} from "../../../http/voice-service-api";
import RemoteChatMessage from "../../../model/consumer/chat/RemoteChatMessage";
import {ChatUtils, ContentType, LocalChatMessage, Voice,} from "../../../model/consumer/chat/chat";
...
import {ajaxService} from "../../../config/sys/ajax";
...
//1 文本，2 图片，3 语音
const valueType = ref<ContentType>(ContentType.TEXT);
...
const changeValueType = () => {
  let type = valueType.value as number;
  type = (type % 3) + 1;
  valueType.value = type as ContentType;
};
...
watch(selfPhotoId, (newVal, oldVal) => {
  if (newVal) {
    let photoId = newVal;
...
const userIdToHistoryId = () => {
  ...
  ajaxService.request<Record<number, number[]>>(config).then((res) => {
    const historyIds: number[] = res.data[pageUserId];
    updateLocalIdToHistoryIdMap(historyIds);
  });
};
  ...
  historyIdToRemoteChatMessage(historyIds).then((res) => {
    historyIds.forEach((n) => {
      ...
      let localChatMessage: LocalChatMessage = {
        remoteId: remoteId,
        contentType: ChatUtils.indexToContentType(
      ...
      switch (localChatMessage.contentType) {
        ...
        case ContentType.IMAGE:
          ...
          fetch(photoUrl).then((res: Response) => {
            localChatMessage.file = new File([res.blob()], 'image');
            updateLocalChatMessageByRemoteId(localChatMessage);
          })
          break;
        default:
...
const sendMessage = () => {
  seenAxios<boolean>({
    method: "POST",
    url: "/seen/sse-v1/chat/two-chat/send",
  }).then((res) => {
  });
};
/**
...
const loadImage = (id: number | null) => {
  nextTick(() => {
    onChatRecordListEndChange();
  });
}
